...
function main(argv) {
  ...
  // Spawn directly without shell to ensure signals (SIGTERM/SIGINT) reach the child process directly
  ...
  child.on("error", (err) => {
    if (err && err.code === "ENOENT") {
      const fallbackChild = spawn(command, args, { stdio: "inherit", env, shell: true });
      for (const signal of ["SIGINT", "SIGTERM", "SIGHUP"]) {
        process.on(signal, () => {
          try {
      ...
      fallbackChild.on("error", (fallbackErr) => {
        console.error(`[with-app-env] failed to run ${command}:`, fallbackErr?.message || fallbackErr);
        process.exit(127);
      });
